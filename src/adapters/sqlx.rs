...
impl SqlxAdapter {
    ...
    fn process_migration_file(
        ...
        if let Some(mig_file) = self.try_suffix_format(path, filename, start_after, check_down)? {
    ...
    fn process_migration_directory(
        ...
        if up_sql.exists() {
            ...
            let metadata = parse_sqlx_directives(&content);
            validate_migration_metadata(&content, &metadata, &up_sql)?;
        ...
        if check_down {
            ...
            if down_sql.exists() {
                let content = fs::read_to_string(&down_sql)?;
                let metadata = parse_sqlx_directives(&content);
                validate_migration_metadata(&content, &metadata, &down_sql)?;
...
fn should_check_migration(start_after: Option<&str>, migration_timestamp: &str) -> bool {
    let Some(start_after) = start_after else {
        return true;
    };

    // Both timestamps should be in YYYYMMDDHHMMSS format (14 digits)
    migration_timestamp > start_after
}
