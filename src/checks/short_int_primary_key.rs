...
impl Check for ShortIntegerPrimaryKeyCheck {
    fn describe(&self) -> Vec<CheckDescription> {
        vec![CheckDescription {
            operation: "PRIMARY KEY with short integer type".into(),
            problem: "Using <type> for primary key column '<column>' on table '<table>' risks ID exhaustion \
                      at <limit> records. <type> can be quickly exhausted in production applications. Changing \
                      the type later requires an ALTER COLUMN TYPE operation that triggers a full table rewrite \
                      with an ACCESS EXCLUSIVE lock, blocking all operations. Duration depends on table size.".into(),
            safe_alternative: "Use BIGINT for primary keys to avoid ID exhaustion:\n\n\
                               Instead of:\n   \
                               ...
                               <column> BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY\n\n\
                               ...
                               use 'safety-assured' to bypass this check.".into(),
            script_path: None,
        }]
    }
