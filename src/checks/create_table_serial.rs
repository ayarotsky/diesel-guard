...
impl Check for CreateTableSerialCheck {
    fn describe(&self) -> Vec<CheckDescription> {
        vec![CheckDescription {
            operation: "CREATE TABLE with SERIAL".into(),
            problem: "Column '<column>' in CREATE TABLE '<table>' uses <type> which is a PostgreSQL \
                      pseudo-type (non-standard SQL). SERIAL also creates a separately-owned sequence \
                      object, which can complicate permissions, dump/restore, and replication workflows.".into(),
            safe_alternative: "Use SQL-standard identity columns: BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY.".into(),
            script_path: None,
        }]
    }
