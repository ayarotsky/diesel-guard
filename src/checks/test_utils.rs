...
/// Assert that a check detects exactly one violation with the given operation name, using explicit MigrationContext.
#[cfg(test)]
#[macro_export]
...
/// Assert that a check allows (finds no violations in) the given SQL, using explicit MigrationContext.
#[cfg(test)]
#[macro_export]
