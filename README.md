...
✓ Extensible with [custom checks](#custom-checks)<br>
...
Need project-specific rules beyond these? See [Custom Checks](#custom-checks).
...
Use `dump-ast` to inspect the AST for any SQL statement. This is the easiest way to discover which fields are available (see [example output](#inspect-the-ast-for-a-sql-statement)):
...
[`safety-assured` blocks](#safety-assured) also suppress custom check violations — any SQL inside a safety-assured block is skipped by all checks, both built-in and custom.
