...
#[test]
fn test_diesel_concurrently_with_metadata_no_transaction() {
    // CONCURRENTLY with metadata.toml run_in_transaction = false
    // should set requires_no_transaction on the MigrationFile
    let temp_dir = TempDir::new().unwrap();
    let migration_dir = temp_dir.path().join("2024_01_01_000000_add_index");
    fs::create_dir(&migration_dir).unwrap();
    ...
    // No violations — CREATE INDEX CONCURRENTLY is safe
    assert_eq!(results.len(), 0);
}
